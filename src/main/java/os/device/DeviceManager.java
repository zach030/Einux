...
public class DeviceManager {
    ...
    public static final int BH_WRITE = 1; // 进行写入处理
    public static final int BH_READ = 2; // 进行读取处理
    public static final int BH_DONE = 4; // 数据最新，写入缓冲区结束时设置
    public static final int BH_BUSY = 8; // 表示缓冲区正使用，无第三方申请
    public static final int BH_WANTED = 16; // 表示缓冲区处于使用中的状态。第三者正等待该缓冲区被释放
    //----------成员-----------------
    ...
    public class BufferOperator {
        ...
        // 获取指定缓冲区
        public BufferHead getBuffer(int devNo, int blockNo) {
            // 0. 从散列队列里取
            // 0.1 获取此设备的缓冲队列
            ...
            for (BufferHead bh : devAllottedBhQueue) {
                ...
                if (bh.getBlockNo() == blockNo) {
                    // 若该bh正在被其他进程使用，且无第三方申请
                    if (bh.getFlag() != BH_BUSY) {
                        // 则设置此缓冲块被第三方等待
                        bh.setFlag(BH_WANTED);
                        // 修改内存位示图
                        modifyStorageBitMap(bh.getBufferNo(), true);
                    ...
                    bh.setFlag(BH_BUSY);
                    // 从空闲队列去掉
                    bufferQueueManager.freeQueue.remove(bh);
                    // 修改缓冲区位示图
                    modifyStorageBitMap(bh.getBufferNo(), true);
        ...
        // 申请缓冲区
        public BufferHead AllotBufferHead(int devNo, int blockNo) {
            // 0. 判断空闲队列是否有空
